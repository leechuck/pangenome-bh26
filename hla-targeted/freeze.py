...
def main():
    ...
    paths=list(ROOT.glob('*.py'))+list(ROOT.glob('*.cpp'))+list(ROOT.glob('*.sbatch'))
    paths += [ROOT/'source'/p for p in ['markers.txt','marker_groups.tsv','marker_design.json','target_definitions.json','training_rows.json','training_profiles.npy','dosage_model.json','tool_version.json','validation_donors.tsv','validation_selection.json','training_C4_genes.tsv']]
    paths += [ROOT/'results'/p for p in ['training_C4_diagnostic_audit.tsv','development_calibration_inputs.tsv','development_dosage_predictions.tsv','development_metrics.tsv','probe_specificity.json']]
