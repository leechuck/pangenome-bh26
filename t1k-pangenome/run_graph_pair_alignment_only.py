"""Exploratory alignment-only ablation; isolated from frozen v1 execution."""
import argparse
import collections
import copy
import hashlib
...
from graph_pair_refinement_alignment_only import coarse,assign_fragment,refine_gene,internal_fragment
...
def run(baseline,joined,reference,calibration,output,internal_pairs=False,pair_specific=False):
    ...
    record=dict(status='running',started=time.time(),donor=bm['donor'],reads_sha256=bm['reads_sha256'],
                ...
                model_sha256=sha(Path(__file__).with_name('graph_pair_refinement_alignment_only.py')),
                ...
                parameters=dict(temperature=10,locus_margin=10,minimum_fragments=20,minimum_gap=10,noise=.01,
                                internal_pairs=internal_pairs,pair_specific=pair_specific,alignment_only=True),
                scope='Exploratory alignment-only graph ablation on exposed donors; no independent validation claim')
