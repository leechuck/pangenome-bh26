"""Aggregate the fixed 64-donor all-read development control."""
import csv
import json
...
if __name__=='__main__':
    output=HERE/'development/all-read-control'
    donors=list(csv.DictReader((HERE/'execution-cohort.tsv').open(),delimiter='\t'))
    rows=[]
    for donor in donors:
        scores,_=run(HERE/'work/graph-recruitment-snapshot',output/'donors'/donor['donor'],
                     donor['donor'],conditions=('all',),quiet=True)
        for row in scores:
            row.update(stratum=donor['stratum'],family=donor['family'])
        rows.extend(scores)
    summary=[]
    for method in ('T1K','ipd_genome','all'):
