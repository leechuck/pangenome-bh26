...
from run_graph_pair_native_locus import run as refine
...
def run(stage,index,assets,freeze_sha256):
    ...
    out=ROOT/'hgsvc-validation-v1/validation-runs/graph-v1';coverage=out/'coverage'/(donor+'.json')
    calibration=out/'library-calibration-v1'/donor
    baseline=ROOT/'hgsvc-validation-v1/validation-runs/genomic-ipd-v1'/donor
    if stage=='coverage':
