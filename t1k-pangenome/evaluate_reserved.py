...
"""
from score_linear_controls import GENES,genomic_truth,truth_slots,prediction,compare
from paired_evaluation import evaluate

METHODS=('T1K','ipd_genome','graph_hprc','graph_hprc_asian')


def score(cohort,results,truth,nomenclature,repetitions=10000,seed=20260918):
    ...
    for donor in cohort:
        ...
        for gene in GENES:
            ...
            for fields in (2,4):
                slots=(genomic_truth(records) if fields==4 else truth_slots(records,'two_field',nomenclature,gene)) if records else None
