...
DP>=10 and alternate depth>=5. It decomposes complex alleles with affine alignment and applies only noncoding
long indels, preserving the source reconstruction outside replacement intervals. Heterozygous/uncertain graph alleles are
...
def long_indels(ref, alt):
    ...
    """
    # Unit-cost edit alignment fragments long repeat-associated gaps into many
    # tiny insertions. Affine gaps represent a contiguous event as one gap.
    from Bio.Align import PairwiseAligner
    aligner=PairwiseAligner(mode='global',match_score=2,mismatch_score=-4,
                          open_gap_score=-6,extend_gap_score=-1)
    coords=aligner.align(ref,alt)[0].coordinates
    events=[]
    for i in range(coords.shape[1]-1):
        t,q=map(int,coords[:,i]); end_t,end_q=map(int,coords[:,i+1])
        if t==end_t and end_q-q>=50 and t>0:
            events.append((t-1,ref[t-1],ref[t-1]+alt[q:end_q]))
        elif q==end_q and end_t-t>=50 and t>0:
            events.append((t-1,ref[t-1:end_t],ref[t-1]))
    return events
