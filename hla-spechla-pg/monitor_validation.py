#!/usr/bin/env python3
"""Collect and score the fixed cohort until completion or an optional deadline."""
import argparse
...
def main():
    ...
    a=p.parse_args(); deadline=datetime.fromisoformat(a.deadline) if a.deadline else None
    if (deadline is not None and deadline.tzinfo is None) or a.interval<30: p.error('Timezone and interval >=30 seconds required')
    ...
    while True:
        ...
        try:
            ...
            record.update(observed_at=now.isoformat(),deadline=deadline.isoformat() if deadline else None,deadline_reached=at_deadline)
            ...
            if at_deadline:
                raise
        time.sleep(min(a.interval,max(1,(deadline-datetime.now(timezone.utc)).total_seconds())) if deadline else a.interval)
