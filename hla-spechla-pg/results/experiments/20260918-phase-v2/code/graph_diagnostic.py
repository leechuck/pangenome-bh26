...
def diagnose(root, out, fold, gene, donor, kind, pairs, threads, timeout):
    ...
    try:
        ...
        with pysam.VariantFile(str(out / 'graph.vcf')) as vcf:
            ...
            sv = [r for r in called_alt if any(abs(len(a) - len(r.ref)) >= 50 for a in r.alts or [])]
        json_write(out / 'summary.json', dict(read_pairs=n, subset='first pairs; performance gate only',
                   projection=metrics, linear=linear_metrics, fragment_distribution=fragment,
                   graph_variant_records=len(records), nonreference_records=len(called_alt),
                   nonreference_indel50_records=len(sv), timing_seconds=timings))
        finish(out, config, {name: sha(out / name) for name in ('preservation.json', 'summary.json', 'reads.gam', 'graph.vcf', 'projected.bam')})
