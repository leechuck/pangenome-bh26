...
ARMS = ('A-replay', 'A-fixed', 'AE-ipd365', 'AE-panel', 'A-pairing', 'AE-ipd365-pairing', 'AE-panel-pairing')
...
def patch_block_linker(source, repair_pairing=False):
    ...
    result = ''.join(lines)
    if repair_pairing:
        # link_type[x][y]: x chooses parallel/crossed phase, y chooses one
        # of the two complementary haplotype pairs. Both y scores belong
        # to the SAME x edge. Upstream's x == y mixes the alternatives.
        tree = ast.parse(result)
        graph = next(x for x in tree.body if isinstance(x, ast.ClassDef) and x.name == 'Construct_Graph')
        edge = next(x for x in graph.body if isinstance(x, ast.FunctionDef) and x.name == 'get_edge')
        conditions = [n for n in ast.walk(edge) if isinstance(n, ast.If)
                      and ast.unparse(n.test) == 'x == y']
        if len(conditions) != 1:
            raise ValueError('Unexpected block edge combination source')
        lines = result.splitlines(keepends=True)
        n = conditions[0]
        lines[n.lineno-1] = lines[n.lineno-1].replace('if x == y:', 'if x == 0:')
        result = ''.join(lines)
...
def prepare(root, release, folds):
    ...
    hashes = {}
    for variant in ('original', 'fixed', 'pairing'):
        ...
        if variant != 'original':
            block.write_text(patch_block_linker(block.read_text(), repair_pairing=variant == 'pairing'))
...
def phase_run(root, release, donor, fold, arm, threads, output=None):
    ...
    variant = 'pairing' if arm.endswith('-pairing') else ('original' if arm == 'A-replay' else 'fixed')
    script = release / 'vendor' / variant / 'script'
    edb = original if arm.startswith('A-') else release / 'db' / f'fold{fold}' / ('ipd365' if arm.startswith('AE-ipd365') else 'panel')
    ...
    try:
        ...
        for gene in GENES:
            command(['python3', script / 'phase_variants.py', '-o', out, '-b', bam, '-s', 'nothing', '-v', vcf,
                     ...
                     '--freq_bias', '.05', '--snp_qual', '.01', '--snp_dp', '5', '--ref', edb / 'HLA' / f'HLA_{gene}' / f'HLA_{gene}.fa',
