...
attempts=[r/f'attempts/arpack-{donor}-{arm}/A',r/f'arms/{arm}/runs/{donor}/A']
...
audit=dict(donor=donor,arm=arm,reason=signature,identical_configuration=True,
...
for variant in ('DogoHLA','DogoHLA-no-graph'):
 p=r/f'arms/{arm}/final/runs/{donor}'/variant
 if p.exists():
...
if __name__ == '__main__':
    ...
    p.add_argument('--arm', choices=['hprc','asian_matched','full'], default='hprc')
    a = p.parse_args()
    if not re.fullmatch(r'(HG|NA)[0-9]+', a.donor) or not a.retry_job.isdigit():
        p.error('Invalid donor or job identifier')
    setup = f'donor={a.donor!r};gene={a.gene!r};retry_job={a.retry_job!r};arm={a.arm!r}\n'
    result = remote('python3 -c ' + shlex.quote(setup+SCRIPT))
    Path(__file__).with_name(f'arpack-{a.donor}'+('' if a.arm=='hprc' else '-'+a.arm)+'-final.json').write_text(result+'\n')
    print(result)
