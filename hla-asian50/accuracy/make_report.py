#!/usr/bin/env python3
"""Write the report from saved aggregate counts and paired bootstrap results."""
from pathlib import Path
import csv
R=Path(__file__).resolve().parent
rows=list(csv.DictReader(open(R/'results/summary.tsv'),delimiter='\t'))
pairs=list(csv.DictReader(open(R/'results/paired_bootstrap.tsv'),delimiter='\t'))
def metric(s,u,k,a):return next(r for r in rows if (r['stratum'],r['universe'],r['variant_class'],r['arm'])==(s,u,k,a))
def pct(r):return f"{float(r['accuracy_pct']):.2f}%"
lines=['# Does the Asian-enriched panel help?', '',
...
'|---|---|---:|---:|---:|']
for s in ['EAS','SAS']:
    for k,label in [('SNV','SNV genotype'),('truth_SV_length','SV-bearing genotype')]:
        f=metric(s,'shared_sites',k,'full');h=metric(s,'shared_sites',k,'hprc')
        b=next(r for r in pairs if (r['stratum'],r['universe'],r['variant_class'],r['baseline'],r['endpoint'])==(s,'shared_sites',k,'hprc','all'))
        lines.append(f"| {s} | {label} | {pct(f)} ({f['correct']}/{f['n']}) | {pct(h)} ({h['correct']}/{h['n']}) | {float(b['full_minus_baseline_pp']):+.2f} ({float(b['ci95_low_pp']):+.2f}, {float(b['ci95_high_pp']):+.2f}) |")
lines+=['',
...
for s in ['EAS','SAS']:
    for k,label in [('SNV','All eligible SNV genotypes'),('truth_SV_length','All eligible SV-bearing genotypes')]:
        f=metric(s,'all_truth',k,'full');h=metric(s,'all_truth',k,'hprc')
        lines.append(f"| {s} | {label} | {pct(f)} ({f['correct']}/{f['n']}) | {pct(h)} ({h['correct']}/{h['n']}) |")
lines+=['',
...
if any(r['arm']=='linear' for r in rows):
...
'## Reproduction and artifacts', '', '![Shared-site gains and coverage losses](results/panel_comparison.png)', '',
