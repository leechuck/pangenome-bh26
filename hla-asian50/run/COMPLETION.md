...
This completes the submitted **call-generation run and output verification**, not the scientific accuracy benchmark. Independent SNV/SV truth and callable masks, the appropriate linear calling/genotyping baselines, classical-HLA translation of graph predictions, paired accuracy comparisons and discordance review remain outstanding. Native HLA databases differ (SpecHLA outputs identify IPD-IMGT/HLA 3.38.0); database provenance must be considered when interpreting comparisons. Initial reads were recruited through reference alignments, so this run does not establish whole-WGS recruitment improvements.
