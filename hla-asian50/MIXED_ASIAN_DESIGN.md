...
No mixed-cohort inference has run. The scripts `select_panel.py` and `check_setup.py` currently reproduce the historical East Asian feasibility experiment only; they must not be used to claim a completed mixed-cohort selection.
