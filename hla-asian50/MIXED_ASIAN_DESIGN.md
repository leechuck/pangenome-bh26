...
**Submitted calling jobs have completed:** see [the initial run](run/README.md) for submitted jobs and remaining validation work. No completed mixed-cohort accuracy result is available. The scripts `select_panel.py` and `check_setup.py` currently reproduce the historical East Asian feasibility experiment only; they must not be used to claim a completed mixed-cohort selection.
